...
```typescript NFC Events
// src/hooks/useNFC.ts
window.onNdefDiscovered = (serial, message, data) => {
    if (config?.onNdef) {
...
Some hooks that require Fully to be available use preflight checks:
...
export const useNFC = (config) => {
    const start = useCallback(() => {
        preflight(); // Throws if not in Fully Kiosk
        fully.nfcScanStart();
    }, []);
...
This pattern is used for features that can't work without Fully Kiosk, like NFC scanning.
